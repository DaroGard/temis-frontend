...
const Card = ({ icon, title }: { icon: React.ReactNode; title: string }) => (
  <motion.div
    className="bg-white rounded-xl shadow-md hover:shadow-xl p-6 text-center transition-transform transform hover:-translate-y-1"
    variants={{
...
const AboutUs = () => {
  return (
    <section
      className="bg-[var(--Tertiary-color)] text-[var(--primary-color)] py-20 px-4 sm:px-6 lg:px-24"
    ...
    >
      <div className="flex flex-col lg:flex-row gap-10 items-center">
        ...
        >
          <h3
            id="aboutus-title"
            className="font-serif text-sm font-bold uppercase mb-4 inline-block border-b-4 border-[var(--pending-color)] pb-1"
          >
            Quiénes somos
          </h3>
