...
export function FileUploadCard({ onFilesChange, onRemoveFile, disabled = false, files = [] }: Props) {
  ...
  return (
    <section className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
      ...
      <div
        {...getRootProps()}
        className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-all duration-200 ${disabled
      ...
      >
        ...
        <div className="flex flex-col items-center gap-4">
          {/* Icono */}
          <div className={`p-3 rounded-full ${disabled ? 'bg-slate-200' : isDragActive ? 'bg-blue-100' : 'bg-slate-200'
            }`}>
            <svg
              className={`w-8 h-8 ${disabled ? 'text-slate-400' : isDragActive ? 'text-blue-500' : 'text-slate-500'
                }`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
          ...
          {/* Texto */}
          <div>
            <p className={`text-base font-medium mb-2 ${disabled ? 'text-slate-400' : isDragActive ? 'text-links' : 'text-slate-700'
              }`}>
              {disabled
            ...
            {!disabled && (
              <button
                type="button"
                className="bg-links hover:bg-blue-700 text-white font-medium py-2 px-6 rounded-md transition-colors duration-200"
              >
