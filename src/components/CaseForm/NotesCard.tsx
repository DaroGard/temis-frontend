    ...
    <section className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
      ...
      <div>
        ...
        <textarea
          ...
          rows={4}
          className={`w-full border rounded-md px-3 py-2 text-slate-900 placeholder-slate-400 resize-none focus:outline-none focus:ring-2 focus:ring-links focus:border-transparent ${errors?.notes ? 'border-warning ring-red-200' : 'border-slate-300'
            }`}
        />
        {errors?.notes && (
          <p className="text-warning text-xs mt-1">{errors.notes.message}</p>
        )}
